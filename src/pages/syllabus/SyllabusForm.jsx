...
import { addSyllabusThunk, updateSyllabusThunk } from "../../redux/slices/syllabusSlice";
...
const SyllabusForm = ({ editData, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editData?.id) {
        await dispatch(updateSyllabusThunk({ id: editData.id, syllabusData: formData }));
      } else {
        await dispatch(addSyllabusThunk(formData));
      }
      onClose?.();
      ...
      <div className="flex-1 flex flex-col" style={{ marginLeft: sidebarWidth }}>
        ...
        <main className="flex-1 p-6 pt-20 overflow-auto">
          <div className="max-w-4xl mx-auto bg-eswhite text-esblack shadow rounded-lg p-6">
            <h2 className="text-2xl font-semibold mb-6">{editData ? "Edit Syllabus" : "Add New Syllabus"}</h2>

            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField label="Title *" name="title" value={formData.title} onChange={handleChange} required />
              <InputField label="Class Name *" name="className" value={formData.className} onChange={handleChange} required />
              ...
              <div className="col-span-2">
                <PrimaryButton type="submit">{editData ? "Update Syllabus" : "Add Syllabus"}</PrimaryButton>
              </div>
            </form>
          </div>
        </main>
